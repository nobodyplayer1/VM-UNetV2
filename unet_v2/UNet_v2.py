...
# from pvtv2 import * # debug use only
...
class UNetV2(nn.Module):
    ...
    def forward(self, x):
        seg_outs = []
        f1, f2, f3, f4 = self.encoder(x) # f1  [2, 64, 64, 64],   f4 [2, 512, 8, 8]

        f1 = self.ca_1(f1) * f1
        f1 = self.sa_1(f1) * f1
        f1 = self.Translayer_1(f1) # f1 [2, 32, 64, 64]

        f2 = self.ca_2(f2) * f2
        f2 = self.sa_2(f2) * f2
        f2 = self.Translayer_2(f2) # f2 [2, 32, 32, 32]

        f3 = self.ca_3(f3) * f3
        f3 = self.sa_3(f3) * f3
        f3 = self.Translayer_3(f3) # f3 [2, 32, 16, 16]

        f4 = self.ca_4(f4) * f4
        f4 = self.sa_4(f4) * f4
        f4 = self.Translayer_4(f4) # f4 [2, 32, 8, 8]

        f41 = self.sdi_4([f1, f2, f3, f4], f4) # [2, 32, 8, 8]
        f31 = self.sdi_3([f1, f2, f3, f4], f3) # [2, 32, 16, 16]
        f21 = self.sdi_2([f1, f2, f3, f4], f2) # [2, 32, 32, 32]
        f11 = self.sdi_1([f1, f2, f3, f4], f1) # [2, 32, 64, 64]
        # 函数seg_outs 输出列表也是 seg_outs 只是名字相同
        seg_outs.append(self.seg_outs[0](f41)) # seg_outs[0] [2, 2, 8, 8]

        y = self.deconv2(f41) + f31
        seg_outs.append(self.seg_outs[1](y)) # seg_outs[1] [2, 2, 16, 16]

        y = self.deconv3(y) + f21
        seg_outs.append(self.seg_outs[2](y)) # seg_outs[2] [2, 2, 32, 32]

        y = self.deconv4(y) + f11
        seg_outs.append(self.seg_outs[3](y)) # seg_outs[3] [2, 2, 64, 64]
...
if __name__ == "__main__":
    pretrained_path = "/raid/code/mamba_all/U-Net_v2/PolypSeg/pvt_pth/pvt_v2_b2.pth"
    model = UNetV2(n_classes=1, deep_supervision=True, pretrained_path=None).cuda()
